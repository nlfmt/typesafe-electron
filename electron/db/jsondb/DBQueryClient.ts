...
> {
     ...
     * @returns The document if found, null otherwise
     */
    find(query: Partial<ToFindQuery<InferModelDef<MDef>[Model]>>): InferModelDef<MDef>[Model] | null {
        const { mdl } = this;
     ...
     */
    findMany(query: Partial<ToFindQuery<InferModelDef<MDef>[Model]>>): InferModelDef<MDef>[Model][] {
        const { mdl } = this;
        const modelSchema = this.db.models[mdl];
     ...
     */
    create(obj: Omit<InferModelDef<MDef>[Model], "_id">): InferModelDef<MDef>[Model] | null {
        const { mdl } = this;
        const modelSchema = this.db.models[mdl];
